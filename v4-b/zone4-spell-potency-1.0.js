/* HAJJEN Zone 4 — +10 base potency for newly crafted Primal spells. */
(()=>{
  const cfg=window.HAJJEN_ZONE_CONFIG||window.HAJJEN_CAMPAIGN_CONFIG;
  const state=window.HAJJEN_CAMPAIGN_STATE;
  const api=window.HAJJEN_SHARED_SPELLBOOK_V2;
  const eventLog=document.getElementById('eventLog');
  if(!cfg||cfg.zone!==4||!state||!api||api.zone!==4||!eventLog)return;
  ...
  function patchPreview(){
    ...
    if(!line||!note||!/\bDAMAGE\b/i.test(line.textContent||'')||/Zone 4 potency/i.test(note.textContent||''))return;
    line.textContent=(line.textContent||'').replace(/(·\s*)(\d+)(\s+DAMAGE\b)/i,(_,before,value,after)=>`${before}${Number(value)+BONUS}${after}`);
    note.textContent=`${note.textContent} Zone 4 potency adds +${BONUS} base damage.`;
  }
  ...
  function selectedRecipe(){
    const data=selectedData(),first=data.find(item=>item.order===1),second=data.find(item=>item.order===2);
    return first&&second?{first,second,key:`${first.name}\u0000${second.name}`}:null;
  }
  function candidateSpell(){
    const recipe=selectedRecipe();if(!recipe||baseDamage[recipe.first.force]==null||modifierBonus[recipe.second.force]==null)return null;
    return {name:spellName[recipe.first.force],force:recipe.first.force,damage:baseDamage[recipe.first.force]+modifierBonus[recipe.second.force]+BONUS,cooldown:baseCooldown[recipe.first.force]||0,recipe};
  }
  function bestKnown(candidate){
    if(!candidate)return null;
    return (api.library||[]).filter(spell=>spell&&!spell.fallback&&keyOf(spell)===keyOf(candidate))
  ...
  function syncUpgradeGuard(){
    guardQueued=false;if(!createBtn)return;
    const candidate=candidateSpell();if(!candidate)return;
    const known=bestKnown(candidate),locked=!!state.combat||!!state.gameOver;
    if(!known){if(!locked)createBtn.disabled=false;createBtn.removeAttribute('data-sbv2-upgrade-blocked');setUpgradeNote('');return;}
    ...
    if(stronger){if(!locked)createBtn.disabled=false;createBtn.removeAttribute('data-sbv2-upgrade-blocked');setUpgradeNote(`UPGRADE · replaces your ${knownShown} damage ${candidate.name}.`,'good');}
    else{createBtn.disabled=true;createBtn.dataset.sbv2UpgradeBlocked='1';setUpgradeNote(candidate.damage===rawDamage(known)?`You already know this ${candidate.name} (${knownShown} damage).`:`You already know a stronger ${candidate.name} (${knownShown} vs ${candidateShown} damage).`,'bad');}
  }
  function scheduleUpgradeGuard(){if(guardQueued)return;guardQueued=true;setTimeout(syncUpgradeGuard,0);}
  function matchingFreshSpell(firstName,secondName){
    return [...(api.library||[])].reverse().find(spell=>{
      if(!spell||spell.zone4PotencyBonus)return false;
      const from=Array.isArray(spell.craftedFrom)?spell.craftedFrom:[];return from[0]===firstName&&from[1]===secondName;
    })||null;
  }
  function applyPotency(spell){
    if(!spell||spell.zone4PotencyBonus)return false;
    spell.damage=(Number(spell.damage)||0)+BONUS;spell.ingredientBonus=(Number(spell.ingredientBonus)||0)+BONUS;spell.zone4PotencyBonus=BONUS;
    const active=(state.spells||[]).find(item=>item?.id===spell.id);
    if(active&&active!==spell){active.damage=spell.damage;active.ingredientBonus=spell.ingredientBonus;active.zone4PotencyBonus=BONUS;}
    api.persist?.();api.render?.();window.HAJJEN_SHARED_ACTION_BAR?.sync?.();window.HAJJEN_SHARED_UI?.sync?.();scheduleUpgradeGuard();return true;
  }
  ...
  new MutationObserver(mutations=>{
    for(const mutation of mutations)for(const node of mutation.addedNodes){
      ...
      const match=/^Created .+? from (.+?) \+ (.+?)\. It was added to the Spell Library\.$/i.exec(text);if(!match)continue;
      const recipeKey=`${match[1]}\u0000${match[2]}`;
      queueMicrotask(()=>{
        if(lastEarlyAppliedRecipe===recipeKey){lastEarlyAppliedRecipe='';node.textContent=`${text} Zone 4 potency: +${BONUS} base damage.`;scheduleUpgradeGuard();return;}
        const spell=matchingFreshSpell(match[1],match[2]);if(applyPotency(spell))node.textContent=`${text} Zone 4 potency: +${BONUS} base damage.`;
      });
    }
  ...
  patchPreview();scheduleUpgradeGuard();

  window.HAJJEN_ZONE4_SPELL_POTENCY={version:'1.0',craftBonus:BONUS,syncUpgradeGuard};
})();
