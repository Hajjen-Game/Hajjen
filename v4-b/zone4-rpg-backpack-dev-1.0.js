/* HAJJEN Zone 4 DEV — RPG Backpack / Equipment prototype.
   ...
   V1.1 is load-order safe: it waits for the shared Profile/Backpack layers and
   restores a visible Backpack launcher if a later UI pass has not exposed one. */
(()=>{
  ...
  function ensureLauncher(modal){
    let button=document.querySelector('.backpack-open');
    ...
    button.textContent='BACKPACK';
    button.setAttribute('aria-label','BACKPACK');
    const help=utility.querySelector('.help-open');
  ...
  function ensureModal(){
    ...
    modal.innerHTML='<div class="modal-card"><div class="modal-heading"><h2>BACKPACK</h2><button type="button" class="close-system">CLOSE</button></div><div class="shared-backpack-body"></div></div>';
    document.body.appendChild(modal);
    modal.querySelector('.close-system')?.addEventListener('click',()=>modal.classList.remove('show'));
    modal.addEventListener('click',event=>{if(event.target===modal)modal.classList.remove('show');});
    return modal;
  }

  function boot(attempt=0){
    if(window.HAJJEN_ZONE4_RPG_BACKPACK||!devRequested())return;

    const cfg=window.HAJJEN_ZONE_CONFIG||window.HAJJEN_CAMPAIGN_CONFIG;
    const state=window.HAJJEN_CAMPAIGN_STATE;
    const rpg=window.HAJJEN_RPG_STATE;
    const shared=window.HAJJEN_SHARED_BACKPACK;
    const modal=shared?.root||document.getElementById('backpackModal')||ensureModal();
    const body=shared?.body||modal?.querySelector('.shared-backpack-body')||modal?.querySelector('.modal-heading')?.nextElementSibling;
    const openButton=ensureLauncher(modal);

    if(!cfg||Number(cfg.zone)!==4||!state||!rpg||!modal||!body||!openButton){
    ...
    modal.classList.add('shared-backpack-modal','zone4-rpg-backpack-modal');
    body.classList.add('shared-backpack-body');

    /* Make the launcher self-sufficient. Existing shared handlers may also run;
       adding the class is idempotent and keeps this prototype visible. */
    ...
    window.HAJJEN_ZONE4_RPG_BACKPACK={
      version:'1.1-visible-load-order-safe',
      render,
    ...
    document.documentElement.dataset.zone4RpgBackpack='ready';
  }
