(()=>{
  ...
  function applyPotency(spell){
    ...
    if(active&&active!==spell){active.damage=spell.damage;active.ingredientBonus=spell.ingredientBonus;active.zone2PotencyBonus=ZONE2_CRAFT_BONUS;}
    api.persist?.();api.render?.();window.HAJJEN_SHARED_ACTION_BAR?.sync?.();window.HAJJEN_SHARED_UI?.sync?.();
    return true;
  }
  ...
  new MutationObserver(mutations=>{
    for(const mutation of mutations)for(const node of mutation.addedNodes){
      ...
      if(!match)continue;
      queueMicrotask(()=>{const spell=matchingFreshSpell(match[1],match[2]);if(applyPotency(spell))node.textContent=`${text} Zone 2 potency: +${ZONE2_CRAFT_BONUS} base damage.`;});
    }
  }).observe(eventLog,{childList:true});

  root?.addEventListener('click',()=>queueMicrotask(patchPreview));
  patchPreview();
  window.HAJJEN_ZONE2_SPELL_POTENCY={version:'1.1',craftBonus:ZONE2_CRAFT_BONUS};
})();
