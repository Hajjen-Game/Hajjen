...
(()=>{
  ...
  function restore(profile){
    ...
    syncing=true;
    rpg().updateProfile(()=>clone(profile),'savegame-profile-restore');
    syncing=false;lastSig='';return true;
  }
  function sync(reason='savegame-profile-sync'){
    ...
    const sig=`${level}|${xp}|${potion}`;
    const current=api.getProfile(),p=current?.progression||{};
    if(sig!==lastSig&&(num(p.level,1)!==level||num(p.xp,0)!==xp||num(p.potion,0)!==potion||num(p.highestZone,1)<4)){
      ...
      api.updateProfile(profile=>{
        ...
        Object.assign(profile.progression,{level,xp,maxHp:baseMax(level),potion,campaignZone:Math.max(4,num(profile.progression.campaignZone,0)),highestZone:Math.max(4,num(profile.progression.highestZone,1))});
        return profile;
      },reason);
      syncing=false;
    }
    ...
    if(dev&&profile){try{localStorage.setItem(DEV_KEY,JSON.stringify({version:1,savedAt:new Date().toISOString(),profile}));}catch{}}
    return profile;
  }
  function loadDev(){
    if(!dev)return null;
    let record=null;try{record=parse(localStorage.getItem(DEV_KEY));}catch{}
    if(record?.profile)restore(record.profile);
  ...
  function applyFresh(profile=get()){
    ...
    state.xp=Math.max(0,num(p.xp,state.xp));state.potion=Math.max(0,num(p.potion,state.potion));
    state.maxHp=baseMax(state.level);state.hp=state.maxHp;state.unsecuredEssence=0;
    window.HAJJEN_ZONE4_RPG_STATS_RUNTIME?.sync?.();return true;
  }

  const saved=loadDev();
  window.HAJJEN_ZONE4_SAVE_PROFILE={version:'1.0',dev,key:dev?DEV_KEY:null,get,restore,sync,applyFresh,saved,baseMax};
