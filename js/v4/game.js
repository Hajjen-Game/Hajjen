(() => {
  ...
  const starterSpell = {
    ...
    force: 'Ember',
    damage: 18,
    desc: 'Deal 18 direct damage. No damage-over-time effect.'
  };
  ...
  const starterPotion = {
    ...
    name: 'Healing Potion',
    heal: 25,
    desc: 'Restore 25 HP instantly. No healing-over-time effect.',
    available: true
  };
  ...
  function refreshTileVisual(r,c){
    ...
    if(s){
      tileEl.classList.add('special',s.type);
      tileEl.dataset.mark = s.mark;
    }
  ...
  function showTile(r,c){
    ...
    if(completed.has(key) && ['mob','guardian','boss'].includes(s.type)){
      tileExtra.textContent = 'Defeated / inactive.';
    }else if(s.type==='boss'){
      tileExtra.textContent = bossUnlocked ? 'Boss Shrine: READY' : 'Boss Shrine: LOCKED';
    }else if(s.type==='ingredient'){
      tileExtra.textContent = 'Standard: reveal 3 → choose 1. Manipulation Cards can change this.';
    }else if(['mob','guardian'].includes(s.type)){
      tileExtra.textContent = danger>=10 ? 'Danger 10+: enemies can also aggro from adjacent tiles.' : 'Direct contact/crossing triggers combat.';
    }else{
  ...
  function renderLoadout(){
    for(let i=0;i<3;i++){
      const el = $(`spellSlot${i+1}`);
      const spell = spellLoadout[i];
      el.classList.toggle('equipped',!!spell);
      el.classList.toggle('empty-slot',!spell);
      el.innerHTML = spell ? `<span>${spell.name.toUpperCase()}</span><small>${spell.damage} direct damage</small>` : 'EMPTY';
    }
    for(let i=0;i<2;i++){
      const el = $(`actionSlot${i+1}`);
      const action = actionLoadout[i];
  ...
  function renderCardDrawCountdown(){
    if(phase==='mulligan'){
      drawCountdown.textContent = 'Card draw starts when the zone begins.';
      return;
    }
    const word = turnsUntilCardDraw===1 ? 'turn' : 'turns';
    drawCountdown.textContent = `${turnsUntilCardDraw} ${word} left to draw new card.`;
  }
  ...
  function clearDirectionSelection(){
    [...dirChoice.querySelectorAll('button')].forEach(x=>x.classList.remove('selected'));
  }

  function displayDir(){
    const map={N:'↑',E:'→',S:'↓',W:'←',CHOOSE:'CHOOSE',WILD:'WILD'};
    dirDiceEl.textContent=dirValue?map[dirValue]:'–';
    clearDirectionSelection();
    if(dirValue==='CHOOSE'){
      dirSub.textContent='Välj riktning';
      dirChoice.classList.add('show');
    }else if(dirValue==='WILD'){
      dirSub.textContent='WILD (0.4: välj riktning)';
      dirChoice.classList.add('show');
    }else{
  ...
  function rollBoth(){
    ...
    chosenDir=null;
    rolled=true;
    moveDiceEl.textContent=moveValue;
    displayDir();
  ...
  function getEffectiveDir(){
    return ['N','E','S','W'].includes(dirValue) ? dirValue : chosenDir;
  }
  ...
  function resetAfterTurn(){
    ...
    dirValue=null;
    chosenDir=null;
    moveDiceEl.textContent='–';
    dirDiceEl.textContent='–';
    dirSub.innerHTML='↑ ↓ ← →<br>CHOOSE / WILD';
    dirChoice.classList.remove('show');
    clearDirectionSelection();
    rollBtn.innerHTML='KASTA<br>TÄRNINGARNA';
    rollBtn.disabled=phase!=='play';
  ...
  function drawCardAfterInterval(){
    ...
    if(!emptySlots.length){
      logline.textContent += ' Card draw skipped: hand is full.';
      return;
    }
    const index = emptySlots[Math.floor(Math.random()*emptySlots.length)];
    const card = drawCardForSlot(index);
    logline.textContent += ` New ${card.category} card drawn: ${card.name}.`;
  }

  function scheduleMobSpawn(){
    const delay = 3 + Math.floor(Math.random()*3);
    mobSpawnTimers.push({turnsLeft:delay});
  }

  function activeEnemyKeys(){
    const keys=[];
    for(const [key,tile] of special.entries()){
      if(['mob','guardian','boss'].includes(tile.type) && !completed.has(key)) keys.push(key);
    }
    return keys;
  }
  ...
  function isNearActiveEnemy(r,c){
    for(const key of activeEnemyKeys()){
      ...
      if(Math.max(Math.abs(er-r),Math.abs(ec-c))<=1) return true;
    }
    return false;
  }
  ...
  function spawnMob(){
    ...
    for(let r=0;r<N;r++){
      for(let c=0;c<N;c++){
        ...
        if(isNearActiveEnemy(r,c)) continue;
        candidates.push([r,c]);
      }
    }
    ...
    spawnSerial++;
    const variants=[
      {name:'Roused Bogling',hp:36,attack:8},
      {name:'Irritated Rootling',hp:38,attack:9},
      {name:'Marsh Prowler',hp:40,attack:9}
    ];
    const enemy=variants[Math.floor(Math.random()*variants.length)];
    special.set(`${r},${c}`,{
      type:'mob',mark:'M',title:enemy.name.toUpperCase(),sub:'Spawned Mob Encounter',
      desc:'The disturbed zone has produced a new mob. Triggers combat when entered or crossed.',
      enemy:{...enemy,spawnId:spawnSerial}
    });
    refreshTileVisual(r,c);
    logline.textContent += ` The disturbed zone spawned ${enemy.name} at ${String.fromCharCode(65+r)}${c+1}.`;
    return true;
  }

  function tickMobSpawns(){
    for(let i=mobSpawnTimers.length-1;i>=0;i--){
      mobSpawnTimers[i].turnsLeft--;
      if(mobSpawnTimers[i].turnsLeft<=0){
        spawnMob();
        mobSpawnTimers.splice(i,1);
      }
    }
  }
  ...
  function finishMovementTurn(){
    ...
    turnsUntilCardDraw--;
    tickMobSpawns();
    if(turnsUntilCardDraw<=0){
      drawCardAfterInterval();
      turnsUntilCardDraw=CARD_DRAW_INTERVAL;
    }
    moving=false;
    resetAfterTurn();
    updateStatus();
  ...
  function findTriggeredEncounter(r,c){
    const directKey=`${r},${c}`;
    const direct=special.get(directKey);
  ...
  function prepareChoiceModal(title,text,hint){
    ...
    const confirm=$('confirmChoiceBtn');
    confirm.hidden=true;
    ...
    return new Promise(resolve=>{
      ...
      options.forEach((ing,index)=>{
        ...
        btn.addEventListener('click',()=>{
          ...
          }else if(chosen.size<pickCount){
            chosen.add(index);
            btn.classList.add('selected');
          }
          $('choiceHint').textContent=`${chosen.size} / ${pickCount} valda`;
          confirm.disabled=chosen.size!==pickCount;
        });
  ...
  async function resolveLanding(){
    ...
    if(s.type==='ingredient') await resolveIngredientTile('landed');
      ...
      hp+=20;
      logline.textContent='Healing Spring: +20% HP instantly.';
    }else if(s.type==='shrine'){
      if(!completed.has(key)){
        shrines++;
        completed.add(key);
        refreshTileVisual(row,col);
      }
      logline.textContent='Shrine aktiverad.';
    }else if(s.type==='boss'){
      logline.textContent=bossUnlocked?'Rootmaw is active. Contact will trigger combat.':'Boss Shrine är låst. Slutför målen först.';
  ...
  rerollMove.addEventListener('click',()=>{
    if(!rolled||moving||phase!=='play') return;
    moveValue=randomMove();
    moveDiceEl.textContent=moveValue;
    logline.textContent='Movement Dice rerolled.';
  });
  ...
  rerollDirection.addEventListener('click',()=>{
    ...
    chosenDir=null;
    displayDir();
    logline.textContent='Direction Dice rerolled.';
  });
  ...
  playCardBtn.addEventListener('click',async()=>{
    ...
    if(card.effect==='keenEye'){
      ...
      moveDiceEl.textContent=moveValue;
      consumeSelectedCard();
      logline.textContent='Measured Step: Movement Dice +1.';
  ...
  function closeInfoModal(){
    ...
    if(returnToCombatPrep && phase==='combat-prep' && pendingEncounter){
      returnToCombatPrep=false;
      renderCombatPrep();
      $('combatPrepModal').classList.add('show');
    }
  }
  ...
  function equipSpellToSlot(spell,index){
    for(let i=0;i<spellLoadout.length;i++){
      if(spellLoadout[i]?.id===spell.id) spellLoadout[i]=null;
    }
    spellLoadout[index]=spell;
    updateStatus();
  }

  function equipActionToSlot(item,index){
    for(let i=0;i<actionLoadout.length;i++){
      if(actionLoadout[i]?.id===item.id) actionLoadout[i]=null;
    }
    if(item.available) actionLoadout[index]=item;
    updateStatus();
  }

  function openSpellbook(targetSlot=null,fromCombatPrep=false){
    if(fromCombatPrep){
      $('combatPrepModal').classList.remove('show');
      returnToCombatPrep=true;
    }
    ...
    const spell=spellLibrary[0];
    const equippedIndex=spellLoadout.findIndex(x=>x?.id===spell.id);
    ...
    showInfo('SPELLBOOK',`
      ...
      <div class="inventory-item">
        <h3>${spell.name}</h3>
        <p>${spell.desc}</p>
        <div class="inventory-status">${equippedIndex>=0?`Currently equipped in Spell Slot ${equippedIndex+1}.`:'Not equipped.'}</div>
        <div class="inventory-actions">
          ${targetSlot!==null?`<button data-spell-equip="${targetSlot}">EQUIP TO SLOT ${targetSlot+1}</button>`:''}
          ${equippedIndex>=0?`<button data-spell-unequip="${equippedIndex}">UNEQUIP</button>`:''}
        </div>
      </div>
      <p><strong>Collected ingredients:</strong></p>${ingredientList}`);
  }
  ...
  function openBackpack(targetSlot=null,fromCombatPrep=false){
    ...
    const item=backpackItems[0];
    const equippedIndex=actionLoadout.findIndex(x=>x?.id===item.id && item.available);
    ...
    showInfo('BACKPACK',`
      ...
      <div class="inventory-item">
        <h3>${item.name}</h3>
        <p>${item.available?item.desc:'This item has been used.'}</p>
        <div class="inventory-status">${equippedIndex>=0?`Currently equipped in Action Slot ${equippedIndex+1}.`:item.available?'Not equipped.':'Unavailable.'}</div>
        <div class="inventory-actions">
          ${item.available&&targetSlot!==null?`<button data-action-equip="${targetSlot}">EQUIP TO SLOT ${targetSlot+1}</button>`:''}
          ${equippedIndex>=0?`<button data-action-unequip="${equippedIndex}">UNEQUIP</button>`:''}
          ${canUseNow?`<button data-use-potion-now="1">USE NOW (+${item.heal} HP)</button>`:''}
        </div>
      </div>
      <p>During combat, only items equipped in Action slots are available.</p>`);
  }

  modalText.addEventListener('click',e=>{
    const spellEquip=e.target.closest('[data-spell-equip]');
    if(spellEquip){
      equipSpellToSlot(starterSpell,Number(spellEquip.dataset.spellEquip));
      closeInfoModal();
      return;
    }
    const spellUnequip=e.target.closest('[data-spell-unequip]');
    if(spellUnequip){
      spellLoadout[Number(spellUnequip.dataset.spellUnequip)]=null;
      updateStatus();
      closeInfoModal();
      return;
    }
    const actionEquip=e.target.closest('[data-action-equip]');
    if(actionEquip){
      equipActionToSlot(starterPotion,Number(actionEquip.dataset.actionEquip));
      closeInfoModal();
      return;
    }
    const actionUnequip=e.target.closest('[data-action-unequip]');
    if(actionUnequip){
      actionLoadout[Number(actionUnequip.dataset.actionUnequip)]=null;
      updateStatus();
      closeInfoModal();
      return;
    }
    if(e.target.closest('[data-use-potion-now]')){
      if(phase==='play'&&!moving&&starterPotion.available){
        hp=Math.min(100,hp+starterPotion.heal);
        starterPotion.available=false;
        for(let i=0;i<actionLoadout.length;i++) if(actionLoadout[i]?.id===starterPotion.id) actionLoadout[i]=null;
        updateStatus();
        logline.textContent=`${starterPotion.name} used from Backpack: +${starterPotion.heal} HP instantly.`;
        closeInfoModal();
      }
    }
  });

  document.querySelectorAll('[data-spell-slot]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if(phase==='combat') return;
      openSpellbook(Number(btn.dataset.spellSlot));
    });
  });

  document.querySelectorAll('[data-action-slot]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if(phase==='combat') return;
      openBackpack(Number(btn.dataset.actionSlot));
    });
  });

  function tacticalCardsInHand(){
    return hand.map((card,index)=>({card,index})).filter(x=>x.card?.category==='Tactical');
  }

  function beginCombatPrep(encounter){
    pendingEncounter=encounter;
    phase='combat-prep';
    prepTacticalIndex=null;
    resetAfterTurn();
    rollBtn.disabled=true;
    $('combatPrepTitle').textContent=`PREPARE: ${encounter.tile.enemy.name.toUpperCase()}`;
    $('combatPrepText').textContent=encounter.reason==='DIRECT CONTACT'
      ? 'You entered or crossed the enemy tile. Your current Spell/Action loadout is carried into combat; you may change it before starting.'
      : 'Danger has activated an adjacent enemy. Your current Spell/Action loadout is carried into combat; you may change it before starting.';
    renderCombatPrep();
    $('combatPrepModal').classList.add('show');
  }
  ...
  function renderPrepLoadout(container,items,type){
    ...
    items.forEach((item,index)=>{
      ...
      if(item && (type==='spell'||item.available)){
        ...
        box.innerHTML=`<strong>${type==='spell'?`SPELL ${index+1}`:`ACTION ${index+1}`}: ${item.name}</strong><small>${item.desc}</small>`;
      }else{
        box.className='prep-option unavailable';
        box.innerHTML=`<strong>${type==='spell'?'SPELL':'ACTION'} ${index+1}: EMPTY</strong><small>No ${type==='spell'?'spell':'action'} equipped.</small>`;
      }
  ...
  function renderCombatPrep(){
    renderPrepLoadout($('prepSpells'),spellLoadout,'spell');
    renderPrepLoadout($('prepPotions'),actionLoadout,'action');
    const tacticalBox=$('prepTactical');
    tacticalBox.innerHTML='';
    const tacticals=tacticalCardsInHand();
    if(!tacticals.length){
      const empty=document.createElement('div');
      empty.className='prep-option unavailable';
      empty.innerHTML='<strong>NO TACTICAL CARD</strong><small>You can still start combat.</small>';
      tacticalBox.appendChild(empty);
    }
    tacticals.forEach(({card,index})=>{
      const btn=document.createElement('button');
      btn.className=`prep-option${prepTacticalIndex===index?' selected':''}`;
      btn.innerHTML=`<strong>${card.name}</strong><small>${card.desc}</small>`;
      btn.onclick=()=>{
        prepTacticalIndex=prepTacticalIndex===index?null:index;
        renderCombatPrep();
      };
      tacticalBox.appendChild(btn);
    });
    const equippedSpells=spellLoadout.filter(Boolean).length;
    const equippedActions=actionLoadout.filter(x=>x?.available).length;
    $('prepHint').textContent=`${equippedSpells} spell(s) · ${equippedActions} action(s) equipped · Tactical: ${prepTacticalIndex===null?'optional':'selected'}`;
    $('startCombatBtn').disabled=equippedSpells===0;
  }
  ...
  $('startCombatBtn').addEventListener('click',()=>{
    ...
    let tactical=null;
    if(prepTacticalIndex!==null && hand[prepTacticalIndex]){
      tactical=hand[prepTacticalIndex];
      hand[prepTacticalIndex]=null;
    }
    const combatSpell=spellLoadout.find(Boolean);
    const combatAction=actionLoadout.find(x=>x?.available) || null;
    combatState={
      encounter:pendingEncounter,
      enemyHp:pendingEncounter.tile.enemy.hp,
      enemyMaxHp:pendingEncounter.tile.enemy.hp,
      turn:1,
      spell:combatSpell,
      action:combatAction,
      tactical,
      blockAvailable:['guard','dodge'].includes(tactical?.tactical),
      firstStrikeAvailable:tactical?.tactical==='firstStrike'
    };
    ...
    renderHand();
    renderCombat();
    $('combatModal').classList.add('show');
  });

  function renderCombat(){
    if(!combatState) return;
    const e=combatState.encounter.tile.enemy;
    $('combatEnemyName').textContent=e.name.toUpperCase();
    $('combatTurn').textContent=`TURN ${combatState.turn}`;
    $('enemyHpFill').style.width=`${Math.max(0,combatState.enemyHp)/combatState.enemyMaxHp*100}%`;
    $('enemyHpText').textContent=`${Math.max(0,combatState.enemyHp)}/${combatState.enemyMaxHp}`;
    $('combatPlayerHpFill').style.width=`${hp}%`;
    $('combatPlayerHpText').textContent=`${hp}%`;
    $('combatSpellBtn').innerHTML=`${combatState.spell.name.toUpperCase()}<br><small>${combatState.spell.damage} direct damage</small>`;
    if(combatState.action?.available){
      $('combatPotionBtn').innerHTML=`${combatState.action.name.toUpperCase()}<br><small>+${combatState.action.heal} HP instantly</small>`;
      $('combatPotionBtn').disabled=false;
    }else{
      $('combatPotionBtn').innerHTML='NO ACTION';
      $('combatPotionBtn').disabled=true;
    }
    const canFlee=combatState.encounter.tile.type==='mob' && combatState.tactical?.tactical==='disengage';
    $('combatFleeBtn').disabled=!canFlee;
    $('combatTacticalInfo').textContent=combatState.tactical?`TACTICAL: ${combatState.tactical.name} — ${combatState.tactical.desc}`:'TACTICAL: None selected';
    updateStatus();
  }

  function enemyAttack(){
    if(!combatState) return;
    const attack=combatState.encounter.tile.enemy.attack;
    if(combatState.blockAvailable){
      combatState.blockAvailable=false;
      $('combatLog').textContent=`${combatState.tactical.name} prevents the incoming attack.`;
    }else{
      hp=Math.max(0,hp-attack);
      $('combatLog').textContent=`Enemy attacks Sharkan for ${attack} damage.`;
    }
    if(hp<=0){
      finishCombat(false);
      return;
  ...
  $('combatSpellBtn').addEventListener('click',()=>{
    ...
    let damage=combatState.spell.damage;
    if(combatState.tactical?.tactical==='focus') damage+=4;
    if(combatState.firstStrikeAvailable){
      damage+=5;
      combatState.firstStrikeAvailable=false;
    }
    combatState.enemyHp=Math.max(0,combatState.enemyHp-damage);
    $('combatLog').textContent=`${combatState.spell.name} deals ${damage} direct damage.`;
    renderCombat();
    if(combatState.enemyHp<=0){
      setTimeout(()=>finishCombat(true),250);
      return;
    }
  ...
  $('combatPotionBtn').addEventListener('click',()=>{
    ...
    hp=Math.min(100,hp+combatState.action.heal);
    combatState.action.available=false;
    for(let i=0;i<actionLoadout.length;i++) if(actionLoadout[i]?.id===combatState.action.id) actionLoadout[i]=null;
    $('combatLog').textContent=`${combatState.action.name}: +${combatState.action.heal} HP instantly.`;
    combatState.action=null;
    renderCombat();
    setTimeout(enemyAttack,250);
  });
  ...
  $('combatFleeBtn').addEventListener('click',()=>{
    ...
    $('combatModal').classList.remove('show');
    logline.textContent=`Disengage: escaped ${combatState.encounter.tile.enemy.name}.`;
    combatState=null;
    phase='play';
    finishMovementTurn();
  });
  ...
  function finishCombat(victory){
    if(!combatState) return;
    const {key,tile}=combatState.encounter;
    $('combatModal').classList.remove('show');
    if(!victory){
      combatState=null;
      phase='gameover';
      ...
      rollBtn.disabled=true;
      updateStatus();
      showInfo('SHARKAN DEFEATED','<p>HP reached 0 during combat. Game-over behavior is still a prototype.</p>');
      return;
    }

    completed.add(key);
    const [er,ec]=key.split(',').map(Number);
    refreshTileVisual(er,ec);

    if(tile.type==='guardian'){
      guardians++;
      logline.textContent=`${tile.enemy.name} defeated. Guardian objective updated.`;
    }else if(tile.type==='boss'){
      logline.textContent='ROOTMAW DEFEATED — Zone prototype complete!';
    }else{
      danger=Math.min(20,danger+2);
      scheduleMobSpawn();
      logline.textContent=`${tile.enemy.name} defeated. Danger +2: the zone has been irritated.`;
    }

    const bossWon=tile.type==='boss';
    combatState=null;
    phase='play';
    finishMovementTurn();
  ...
  $('helpBtn').addEventListener('click',()=>showInfo('HAJJEN V4 — HELP',`
    <p><strong>Movement:</strong> roll Movement Dice + Direction Dice. Edge collisions bounce and remaining steps continue in the opposite direction. CHOOSE/WILD direction buttons visibly mark the selected direction.</p>
    ...
    <p><strong>Card draw:</strong> every 3 completed turns, one new card is drawn if any hand slot is empty. The empty slot keeps its category. Active Manipulation Cards do not block card draw.</p>
    ...
    <p><strong>Loadout:</strong> Sharkan starts with Ember Bolt and one Healing Potion. Spell/Action slots persist until you change them. Clicking any Spell slot opens Spellbook; clicking any Action slot opens Backpack.</p>
    <p><strong>Backpack:</strong> outside combat you may use a healing item from Backpack from any tile. During combat you only have Actions that were equipped in Action slots.</p>
    <p><strong>Combat:</strong> enemies trigger if Sharkan lands on or moves through their tile. Your current loadout carries into turn-based combat, and pre-combat preparation lets you change it and select up to one Tactical Card.</p>
    <p><strong>Mob kills:</strong> defeating a normal mob adds +2 Danger on top of the normal turn increase and may cause another mob to spawn after several turns.</p>
  ...
  document.querySelectorAll('[data-modal]').forEach(btn=>{
    btn.addEventListener('click',()=>{
      if(btn.dataset.modal==='Spellbook') openSpellbook();
      else if(btn.dataset.modal==='Backpack') openBackpack();
