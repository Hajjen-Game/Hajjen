(()=>{
  ...
  function candidateSpell(){
    ...
    return {
      ...
      damage:baseDamage[recipe.first.force]+modifierBonus[recipe.second.force]+ZONE3_CRAFT_BONUS,
  ...
  // Apply Zone 3 potency before the production strongest-version pruner gets a
  ...
  new MutationObserver(mutations=>{
    for(const mutation of mutations)for(const node of mutation.addedNodes){
      ...
      queueMicrotask(()=>{
        ...
        if(applyPotency(spell))node.textContent=`${text} Zone 3 potency: +${ZONE3_CRAFT_BONUS} base damage.`;
      });
    }
