(()=>{
  ...
  left?.classList.remove('side','left-side');left?.classList.add('leftcol');
  right?.classList.remove('side','right-side');right?.classList.add('rightcol');
  ...
  if(manipPanel){
    ...
    const h=manipPanel.querySelector('h2');if(h)h.textContent='MANIPULATION';
    if(!manipPanel.querySelector('.panel-note')){
      const note=document.createElement('p');note.className='panel-note';note.textContent='Zone 2: four world-control cards.';h?.after(note);
    }
    $('manipCards')?.classList.add('mini-cards');
  }
  const decorateManip=()=>{
    $('manipCards')?.querySelectorAll('.card').forEach(card=>card.classList.add('mini-card'));
  };
  decorateManip();
  if($('manipCards'))new MutationObserver(decorateManip).observe($('manipCards'),{childList:true,subtree:true});
  ...
  if(legend)legend.innerHTML='<span><i class="legend-dot ingredient-color">✿</i> Spell ingredient</span><span><i class="legend-dot ingredient-color">⚗</i> Potion ingredient</span><span><i class="legend-dot mob-color">☠</i> Mob</span><span><i class="legend-dot elite-color">⚔</i> Elite</span><span><i class="legend-dot boss-color">♛</i> Boss</span>';
  ...
  if(status){
    ...
    status.innerHTML=`<h2>SHARKAN</h2>
      ...
      <div class="danger-expanded">
        ...
        <div class="pressure-grid"><div><span>Visible</span><strong id="visibleText"></strong></div><div><span>Next ambient Danger</span><strong id="clockText"></strong></div><div><span>Enemy power</span><strong id="powerText"></strong></div><div><span>Zone level cap</span><strong>${cfg.levelCap}</strong></div></div>
        <p class="panel-note">Every 3 movement steps adds +1 Danger. Harvesting adds +1. Normal mob kills add +2.</p>
      </div>`;
  }
  ...
  if(tileInfo&&viewport){
    ...
    world?.querySelectorAll('.tile').forEach(tile=>{
      ...
      tile.addEventListener('click',()=>{if(matchMedia('(hover: none)').matches){showTip();tipTimer=setTimeout(hideTip,1800);}});
    });
  }
  ...
  if(spellPanel){
    ...
    const title=document.createElement('h2');title.textContent='SPELLBOOK';
    heading.appendChild(title);
    if(craftBtn){craftBtn.classList.add('small-btn');heading.appendChild(craftBtn);}
    oldHeading?.replaceWith(heading);
    $('spellGrid')?.classList.add('spell-list');
    const res=$('spellResources');
    if(res){res.classList.add('inventory-line');res.innerHTML='<strong>INGREDIENTS</strong><span id="spellIngredientText">None</span>';}
  }

  function modalShell(id,titleText){
    const modal=document.createElement('div');modal.id=id;modal.className='modal system-modal';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');
    ...
    const close=document.createElement('button');close.type='button';close.className='close-system';close.textContent='CLOSE';
    heading.append(h,close);const body=document.createElement('div');card.append(heading,body);modal.appendChild(card);document.body.appendChild(modal);
    const closeModal=()=>modal.classList.remove('show');close.addEventListener('click',closeModal);modal.addEventListener('click',e=>{if(e.target===modal)closeModal();});
  ...
  if(spellPanel){spellbook.body.appendChild(spellPanel);spellPanel.classList.add('spellbook-restored');}
  ...
  help.body.innerHTML=`<div class="help-copy"><strong>ZONE 2 GOAL</strong><ul><li>Create a Healing Potion from Moonleaf + Clearwater.</li><li>Defeat 4 normal mobs and both Guardians to unlock the Zone 2 boss.</li><li>The world is wider than Zone 1; the camera follows Sharkan horizontally.</li></ul><strong>DANGER</strong><ul><li>Movement, harvesting and combat raise Danger just like Zone 1.</li><li>Higher Danger increases spawn pressure and enemy scaling.</li></ul><strong>BACKPACK</strong><ul><li>Unused spell ingredients carry between zones.</li><li>Healing Potions and crafted spells also carry forward.</li></ul><strong>SPELLS</strong><ul><li>Your Zone 1 spells remain available.</li><li>Collect two ingredients to create another spell if you still have a free crafted-spell slot.</li></ul></div>`;

  const cardsHud=document.createElement('div');cardsHud.className='cards-hud';
  const deckRow=document.createElement('div');deckRow.className='deck-row';
  deckRow.innerHTML='<div class="deck-pile manipulation"><strong>MANIPULATION</strong><span id="manipDeckCount">4 / 4 READY</span></div><div class="deck-pile enhancement locked"><strong>ENHANCEMENT</strong><span>LOCKED · ZONE 2</span></div><div class="deck-pile tactical locked"><strong>TACTICAL</strong><span>LOCKED</span></div>';
  cardsHud.appendChild(deckRow);
  if(manipPanel){const h=manipPanel.querySelector('h2');if(h)h.textContent='HAND — MANIPULATION';cardsHud.appendChild(manipPanel);}

  const actionHud=document.createElement('div');actionHud.className='action-hud';
  actionHud.innerHTML='<div class="action-hud-title"><span>ACTION BAR</span><span>3 SPELLS · 2 ACTIONS</span></div><div id="actionbar" class="actionbar"><button type="button" class="action-slot spell-slot empty" data-action-spell="0">EMPTY SPELL</button><button type="button" class="action-slot spell-slot empty" data-action-spell="1">EMPTY SPELL</button><button type="button" class="action-slot spell-slot empty" data-action-spell="2">EMPTY SPELL</button><div class="action-divider"></div></div>';
  const actionbar=actionHud.querySelector('#actionbar');
  if(potionBtn){potionBtn.classList.add('action-slot');actionbar.appendChild(potionBtn);}
  const emptyAction=document.createElement('button');emptyAction.type='button';emptyAction.className='action-slot action-empty empty';emptyAction.disabled=true;emptyAction.textContent='EMPTY ACTION';actionbar.appendChild(emptyAction);

  const utility=document.createElement('div');utility.className='utility-hud';
  const spellbookBtn=document.createElement('button');spellbookBtn.type='button';spellbookBtn.className='spellbook-open';spellbookBtn.textContent='SPELLBOOK';
  const backpackBtn=document.createElement('button');backpackBtn.type='button';backpackBtn.className='backpack-open';backpackBtn.textContent='BACKPACK';
  const helpBtn=document.createElement('button');helpBtn.type='button';helpBtn.className='help-open';helpBtn.textContent='HELP';
  if(resetBtn)utility.append(spellbookBtn,backpackBtn,helpBtn,resetBtn);else utility.append(spellbookBtn,backpackBtn,helpBtn);
  spellbookBtn.addEventListener('click',spellbook.open);backpackBtn.addEventListener('click',backpack.open);helpBtn.addEventListener('click',help.open);

  if(bottom){bottom.innerHTML='';bottom.append(cardsHud,actionHud,utility);}
  ...
  function syncDeck(){
    const buttons=[...($('manipCards')?.querySelectorAll('button')||[])],ready=buttons.filter(b=>!b.disabled&&/^PLAY/.test(b.textContent||'')).length;
    const count=$('manipDeckCount');if(count)count.textContent=`${ready} / 4 READY`;
  }
  function syncBackpack(){
    const potion=$('backpackPotionText');if(potion)potion.textContent=`${state.potion} left · +30 HP`;
    const spellIng=$('backpackSpellIngredients');if(spellIng)spellIng.textContent=state.spellIngredients.length?state.spellIngredients.map(i=>`${i.name} (${i.force})`).join(' · '):'None';
    const potionIng=$('backpackPotionIngredients');if(potionIng)potionIng.textContent=state.potionIngredients.length?state.potionIngredients.join(' · '):'None';
    const line=$('spellIngredientText');if(line)line.textContent=state.spellIngredients.length?state.spellIngredients.map(i=>`${i.name} (${i.force})`).join(' · '):'None';
    const use=$('backpackUsePotion');if(use)use.disabled=!potionBtn||potionBtn.disabled;
  }
  $('backpackUsePotion')?.addEventListener('click',()=>{potionBtn?.click();syncBackpack();});
  craftBtn?.addEventListener('click',spellbook.close);

  function syncStatus(){
    const hpText=$('hpText'),xpText=$('xpText'),levelText=$('levelText'),dangerText=$('dangerText');
    if(hpText)hpText.textContent=`${state.hp} / ${state.maxHp}`;
    if(levelText)levelText.textContent=state.level;
    const thresholds={4:120,5:180,6:250,7:330};const prev=thresholds[state.level]||120,next=thresholds[state.level+1]||prev;
    ...
    const hpFill=$('hpFill');if(hpFill)hpFill.style.width=`${state.hp/state.maxHp*100}%`;
    const xpFill=$('xpFill');if(xpFill){const pct=state.level>=cfg.levelCap?100:Math.max(0,Math.min(100,(state.xp-prev)/(next-prev)*100));xpFill.style.width=`${pct}%`;}
    const dangerFill=$('dangerFill');if(dangerFill)dangerFill.style.width=`${state.danger*5}%`;
    ...
    const dangerState=$('dangerState');if(dangerState){dangerState.className=`danger-state ${tier}`;dangerState.textContent=state.zoneCleared?'CLEARED':tier.toUpperCase();}
    ...
    const rule=$('dangerRule');if(rule)rule.innerHTML=`<strong>${state.zoneCleared?'CLEARED':info[0]}</strong><span>${state.zoneCleared?'No more enemy pressure in this zone.':info[1]}</span>`;
  }

  function syncPotionButton(){
    if(!potionBtn)return;
    const desired=`USE HEALING POTION <small>${state.potion} left · +30 HP</small>`;
  ...
  if(potionBtn)new MutationObserver(()=>{syncPotionButton();syncBackpack();}).observe(potionBtn,{childList:true,subtree:true,characterData:true,attributes:true});

  const tick=()=>{syncStatus();syncBackpack();syncDeck();syncActionbar();syncPotionButton();};
  tick();
  ...
  window.addEventListener('keydown',e=>{
    ...
    if(!anySystem)return;
    if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','w','W','a','A','s','S','d','D'].includes(e.key)){e.preventDefault();e.stopImmediatePropagation();}
    if(e.key==='Escape'){spellbook.close();backpack.close();help.close();}
  },true);
})();
