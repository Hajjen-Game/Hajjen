/* HAJJEN Zone 4 DEV — proven V1 Manipulation reward pool + Smoke Trail.
   Keeps the exact ten-card pool that was previously visible in the working V1
   test and only adds Smoke Trail as card 11. No movement/effect overrides. */
(()=>{
  const cfg=window.HAJJEN_ZONE_CONFIG||window.HAJJEN_CAMPAIGN_CONFIG;
  const api=window.HAJJEN_ZONE4_CARD_REWARD_TEST;
  if(!window.HAJJEN_ZONE4_DEV_MODE||!cfg||cfg.zone!==4||!api?.decks)return;
  ...
  window.HAJJEN_ZONE4_MANIPULATION_POOL11={version:'1.1-v1-pool-plus-smoke-trail',pool:clonePool(pool)};
})();
