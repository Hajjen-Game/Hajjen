/* HAJJEN Zone 4 — same ambient-spawn telegraph/presentation used in Zones 2–3. */
(()=>{
  ...
  if(!cfg||cfg.zone!==4||!state||!(entities instanceof Map)||!world||!toastArea||window.HAJJEN_ZONE4_SPAWN_PARITY)return;
  ...
  style.dataset.zone4SpawnParity='1.0';
  style.textContent=`
  ...
  function materialize(e){
    ...
    if(state.zoneCleared||state.gameOver){cancelPending(e);return;}
    const tile=tileAt(e.r,e.c);
  ...
  world.addEventListener('click',clickBlock,true);

  window.HAJJEN_ZONE4_SPAWN_PARITY={
    version:'1.0-zones2-3-visual-parity',
    get pending(){return pending.size;},
    restore(){
      toastObserver.disconnect();
      window.removeEventListener('keydown',keyBlock,true);
      world.removeEventListener('click',clickBlock,true);
      pending.forEach(cancelPending);style.remove();
    }
  };
})();
