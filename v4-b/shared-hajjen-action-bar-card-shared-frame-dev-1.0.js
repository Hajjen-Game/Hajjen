/* HAJJEN DEV Action Bar card frames.
   Draws the SAME shared vector frame geometry used by Objectives, Sharkan,
   Event Log and the outer Action Bar, but as one passive overlay above the
   existing Action Bar grid. It never rewrites spell/potion button contents,
   never observes mutations and never intercepts input. */
(()=>{
  ...
  gradient.id='hajjenActionCardSharedGoldDev';
  ...
  const frames=slots.map((slot,index)=>{
    ...
    gold.setAttribute('stroke','url(#hajjenActionCardSharedGoldDev)');
