/* HAJJEN shared vector title plaque — DEV + gameplay-safe Zone 3 production.
   V2.3: approved green plaques remain on SHARKAN/EVENT LOG, while ACTION BAR,
   CARD DECKS and HAND share the exact same muted-navy plaque variant.
   Production Zone 3 starts only after the safe redesign bridge signals ready. */
(()=>{
  const params=new URLSearchParams(location.search);
  const safeProduction=()=>document.documentElement.dataset.hajjenPromoted==='zone3-safe';
  const shouldRun=()=>params.get('dev')==='1'||safeProduction();

  function start(){
    if(window.HAJJEN_VECTOR_TITLE_PLAQUE?.version==='2.3')return;
    ...
    function mount(panel,index=0,options={}){
      ...
      heading.dataset.vectorTitlePlaqueSource=`${variant}-gold-2.3`;

      const layer=document.createElement('div');
      layer.className=`hajjen-vector-title-plaque-layer hajjen-vector-title-plaque-${variant}`;
      layer.dataset.vectorTitlePlaque=`${variant}-gold-2.3`;
    ...
    window.HAJJEN_VECTOR_TITLE_PLAQUE={version:'2.3',mount,buildPlaque};
  }

  if(shouldRun())start();
  else document.addEventListener('hajjen-ui-redesign-promoted',()=>{
    if(shouldRun())start();
  },{once:true});
})();
