...
(()=>{
  ...
  function ensureButtons(modal){
    ...
    let restart=section.querySelector('button:not([data-dev-reset-l10])');
  ...
  function createFallback(){
    ...
    if(!modal){
      ...
      modal.querySelector('button')?.addEventListener('click',restartExpedition);
    }
    ensureButtons(modal);
  ...
  window.HAJJEN_ZONE4_DEV_RESET_CONTROLS={version:'1.0',resetToLevel10,restartExpedition,showDeathControls,stop:()=>clearInterval(timer)};
})();
