/* HAJJEN Zone 3 DEV — Spellbook V4 strongest-version rule.
   Keeps only the strongest crafted version of each spell family in the library.
   If a stronger version is crafted while an older version is loaded, the loaded
   spell is upgraded in place so its slot/id (and enchantment bindings) survive. */
(()=>{
  ...
  function boot(){
    ...
    if(!api?.root||!Array.isArray(api.library)||!api.state){
    ...
    function copyCraftCore(target,source){
      ...
      target.ingredientBonus=source.ingredientBonus;
    ...
    function normalizeLibrary({notify=false}={}){
      ...
      groups.forEach((items,key)=>{
        ...
        let strongest=items[0];
        items.slice(1).forEach(item=>{if(better(item,strongest))strongest=item;});
        ...
        const oldDamage=rawDamage(canonical);
        const oldCooldown=rawCooldown(canonical);

        if(better(strongest,canonical)){
          copyCraftCore(canonical,strongest);
          upgrades.push({name:canonical.name,oldDamage,newDamage:rawDamage(canonical),oldCooldown,newCooldown:rawCooldown(canonical)});
        }

        /* If more than one copy is currently loaded, keep their slot identities
           but upgrade every loaded copy to the strongest core so no weaker loaded
           version remains. The library itself still keeps only one canonical card. */
        activeMatches.slice(1).forEach(active=>{
          if(better(strongest,active))copyCraftCore(active,strongest);
        });
      ...
      library.forEach(spell=>{
        if(!spell||spell.fallback){next.push(spell);return;}
        const key=keyOf(spell);
        if(emitted.has(key))return;
        emitted.add(key);
        next.push(canonicalByKey.get(key)||spell);
      });
      ...
      if(next.length!==library.length||upgrades.length){
        ...
        window.HAJJEN_ZONE3_ENCHANTMENTS?.sync?.();
        if(notify&&upgrades.length){
          const last=upgrades[upgrades.length-1];
          addToast(`${last.name.toUpperCase()} UPGRADED · ${last.oldDamage} → ${last.newDamage}`);
        }
      }
      return upgrades;
    }
    ...
    function selectedForces(){
      ...
      return {
        first:selected(1)?.querySelector('small')?.textContent?.trim()||'',
        second:selected(2)?.querySelector('small')?.textContent?.trim()||''
      };
    }

    function candidateSpell(){
      const {first,second}=selectedForces();
      if(!first||!second||baseDamage[first]==null||modifierBonus[second]==null)return null;
      return {name:spellName[first],force:first,damage:baseDamage[first]+modifierBonus[second]};
    }
    ...
    function bestKnown(candidate){
      ...
      return library
        ...
        .reduce((best,spell)=>!best||better(spell,best)?spell:best,null);
    }
    ...
    function syncCraftGuard(){
      ...
      if(candidate.damage<=rawDamage(known)){
        ...
        setUpgradeNote(candidate.damage===rawDamage(known)
          ?`You already know this ${candidate.name} (${knownShown} damage).`
          :`You already know a stronger ${candidate.name} (${knownShown} vs ${candidateShown} damage).`,'bad');
      }else{
        createBtn.removeAttribute('data-sbv2-upgrade-blocked');
        const levelBonus=(Math.max(1,Number(state.level)||1)-1)*4;
    ...
    /* Clean up old duplicate library entries immediately, including campaigns
    ...
    function scheduleGuard(){
      if(queued)return;
      queued=true;
      queueMicrotask(()=>{queued=false;syncCraftGuard();});
    }
    ...
    /* The shared create handler runs first. Afterwards collapse the newly-created
       version into the existing spell family and upgrade any loaded copy in place. */
    createBtn.addEventListener('click',()=>{
      queueMicrotask(()=>{
        normalizeLibrary({notify:true});
        syncCraftGuard();
      });
    });

    window.HAJJEN_SPELLBOOK_DEV_V4={version:'4.0',normalizeLibrary,syncCraftGuard};
  }
