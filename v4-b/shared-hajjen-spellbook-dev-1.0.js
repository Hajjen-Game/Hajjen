/* HAJJEN Zone 3 DEV — Spellbook visual prototype V1.
   Presentation only: copies Action Bar surface, applies spell-family PNG icons,
   and mounts Shared Vector Frames on the Spellbook window + major sections. */
(()=>{
  ...
  const forceIconFiles={
    growth:'thorn-bloom.png',
    ember:'cinder-burst.png',
    flow:'tide-lash.png',
    stone:'stone-breaker.png',
    gale:'razor-gust.png',
    aether:'rift-pulse.png'
  };
  ...
  function mountFrame(target,frameClass){
    ...
    gradient.id=`hajjenSpellbookGold-${++frameSerial}`;
    ...
    function render(){
      const w=Math.max(80,target.clientWidth||0);
      const h=Math.max(80,target.clientHeight||0);
  ...
  function setSpellIcon(node,name){
    ...
    if(file){
      node.style.setProperty('--sb-spell-icon',`url("assets/action-bar-icons/${file}")`);
      node.dataset.spellbookIcon=file;
    }else{
      node.style.removeProperty('--sb-spell-icon');
      delete node.dataset.spellbookIcon;
    }
  }
  ...
  function syncIcons(){
    ...
    root.querySelectorAll('.sbv2-force').forEach(node=>{
      ...
      if(file){
        node.style.setProperty('--sb-force-icon',`url("assets/action-bar-icons/${file}")`);
        node.dataset.forceIcon=file;
      }
    });
  }
  ...
  function scheduleSync(){
    ...
    syncRaf=requestAnimationFrame(()=>{
      ...
      syncIcons();
      outerFrame?.render?.();
      sectionFrames.forEach(item=>item?.render?.());
    });
  }
  ...
  window.HAJJEN_SPELLBOOK_DEV={
    version:'1.0',modal,card,root,outerFrame,sectionFrames,
    sync(){copyActionBarSurface();syncIcons();outerFrame?.render?.();sectionFrames.forEach(item=>item?.render?.());}
  };
})();
