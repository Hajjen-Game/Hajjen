...
function updatePortalVisibility() {
  document.getElementById("emberPortal")?.classList.toggle("unlocked", state.introDone);
  document.getElementById("growthPortal")?.classList.toggle("unlocked", state.introDone);
}
...
function checkContext() {
  ...
  if (state.zone === "intro") {
    if (dist(980, 528) < 115) {
      hint.textContent = state.introDone ? "Professor Morrow is watching the portals" : "Click Professor Morrow to talk";
    } else if (dist(242, 726) < 130) {
      hint.textContent = state.introDone ? "Enter the Ember portal" : "Professor Morrow should explain the crossroads first";
    } else if (dist(1455, 720) < 130) {
      hint.textContent = state.introDone ? "Enter the Growth portal" : "Professor Morrow should explain the crossroads first";
    } else {
      hint.textContent = "WASD to walk · SPACE to hop · I backpack · B spellbook · F2 anchor debug";
    }

    if (state.introDone && dist(242, 726) < 62) zoneSystem.goZone("ember");
    if (state.introDone && dist(1455, 720) < 62) zoneSystem.goZone("growth");
    return;
  }

  if (state.zone === "ember") {
    if (dist(1390, 465) < 125) hint.textContent = "Click the Ember Keeper to learn about Ember";
    else if (dist(190, 165) < 115) hint.textContent = "Return through the Ember portal";
    else hint.textContent = "Explore Ember · stay on the paths · SPACE to hop · F2 anchor debug";

    if (dist(190, 165) < 58) zoneSystem.returnToIntro("ember");
    return;
  }

  if (state.zone === "growth") {
    if (dist(960, 965) < 120) hint.textContent = "Return through the Growth portal";
    else hint.textContent = "Explore Growth Grove · stay on the paths · Growth Keeper coming next";

    if (dist(960, 965) < 58) zoneSystem.returnToIntro("growth");
  }
}
...
document.getElementById("otter").addEventListener("click", () => {
  ...
  if (state.introDone) {
    dialogue.start([
      ["Professor Morrow", "The portals are awake now. Ember burns to the west, while Growth waits to the east."],
      ["Professor Morrow", "Choose whichever force calls to you first. You will need to understand all six before your Spellbook can reach its full potential."]
    ]);
    return;
  }
  ...
  dialogue.start([
    ...
    ["Professor Morrow", "For now, Ember and Growth are stable enough to reach. Follow either path and step through its portal."],
    ["Sharkan", "Two places to start. Okay... let's see what these forces can teach me."]
  ], () => {
    state.introDone = true;
    updatePortalVisibility();
    toast("The Ember and Growth portals have awakened.");
  });
});
  ...
  dialogue.start(lines, () => {
    if (!already) {
      state.inventory.push({
        ...
        name: "Emberstone",
        force: "Ember",
        traits: ["Power", "Burn", "Duration"],
        desc: "A dense stone holding a steady inner flame."
      });
...
function loop(t) {
  ...
  if (!modalOpen()) {
    ...
    if (state.moving) {
      ...
      let nx = state.x + dx * PLAYER.speed * dt;
      let ny = state.y + dy * PLAYER.speed * dt;
      ...
      if (allowed(state.zone, nx, ny)) {
        ...
        if (allowed(state.zone, nx, state.y)) {
          const p = applyPathMagnet(state.zone, nx, state.y);
          state.x = p[0];
          state.y = p[1];
        }
        if (allowed(state.zone, state.x, ny)) {
          const p = applyPathMagnet(state.zone, state.x, ny);
          state.x = p[0];
          state.y = p[1];
        }
      }
