...
const STORAGE_KEY = "arena3v3-bindings-v1";
...
export class InputManager {
  ...
  saveBindings() { localStorage.setItem(STORAGE_KEY, JSON.stringify(this.bindings)); }
  onKeyDown(event) {
    if (this.capture) {
      event.preventDefault();
      if (event.code === "Escape") {
        const callback = this.capture.callback; this.capture = null; callback(null); return;
      }
      this.rebind(this.capture.action, event.code);
      const callback = this.capture.callback; this.capture = null; callback(event.code); return;
    }
    this.keysDown.add(event.code);
    const action = Object.entries(this.bindings).find(([, code]) => code === event.code)?.[0];
    if (action?.startsWith("spell")) { event.preventDefault(); this.actionHandler?.(action); }
  }
  setActionHandler(handler) { this.actionHandler = handler; }
  isHeld(action) { return this.keysDown.has(this.bindings[action]); }
  movementVector() {
    return {
  ...
  captureNext(action, callback) { this.capture = { action, callback }; }
  rebind(action, code) {
  ...
  reset() { this.bindings = { ...DEFAULT_BINDINGS }; this.saveBindings(); }
  label(action) { return printableCode(this.bindings[action]); }
}
