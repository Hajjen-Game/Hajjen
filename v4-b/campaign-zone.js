(()=>{
  ...
  root.innerHTML=`<div class="campaign-app">
    ...
    <aside class="side left-side">
      <section class="panel"><h2>OBJECTIVES</h2><div class="quest intro"><i>✦</i><div><strong>${cfg.introQuest}</strong><small id="introQuest">NOT COMPLETE</small></div></div><div class="quest"><i>☠</i><div><strong>Cull the Wilds</strong><small id="mobQuest">Mobs: 0 / 4</small></div></div><div class="quest"><i>⚔</i><div><strong>Break the Guardians</strong><small id="eliteQuest">Elites: 0 / 2</small></div></div><div class="quest"><i>♛</i><div><strong>${cfg.bossTitle}</strong><small id="bossQuest">Boss: LOCKED</small></div></div></section>
  ...
  function maybeAggro(){if(state.danger<10&&cfg.zone!==4)return;for(let dr=-1;dr<=1;dr++)for(let dc=-1;dc<=1;dc++){if(!dr&&!dc)continue;const e=entities.get(key(state.row+dr,state.col+dc));if(e&&!e.completed&&(e.type==='mob'||e.type==='elite')){toast('ADJACENT AGGRO','danger');startCombat(e,true);return;}}}
  ...
  function updateQuests(){const was=state.bossUnlocked;state.bossUnlocked=state.mobKills>=4&&state.eliteKills>=2;$('mobQuest').textContent=`Mobs: ${Math.min(state.mobKills,4)} / 4`;$('eliteQuest').textContent=`Elites: ${Math.min(state.eliteKills,2)} / ${ELITE_TARGET??2}`;$('bossQuest').textContent=state.bossKilled?'Boss: DEFEATED':state.bossUnlocked?'Boss: READY':'Boss: LOCKED';$('introQuest').textContent=state.introComplete?'COMPLETE':'NOT COMPLETE';if(state.bossUnlocked&&!was)toast('BOSS UNLOCKED','reward');}
