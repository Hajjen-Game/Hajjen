...
(()=>{
  ...
  function boot(){
    ...
    function candidateSpell(){const {first,second}=selectedForces();if(!first||!second||baseDamage[first]==null||modifierBonus[second]==null)return null;return {name:spellName[first],force:first,damage:baseDamage[first]+modifierBonus[second]+zoneCraftBonus};}
    ...
    function syncCraftGuard(){
      ...
      if(!candidate){createBtn.removeAttribute('data-sbv2-upgrade-blocked');setUpgradeNote('');return;}
      if(!known){
        if(!state.combat&&!state.gameOver)createBtn.disabled=false;
        createBtn.removeAttribute('data-sbv2-upgrade-blocked');setUpgradeNote('');return;
      }
