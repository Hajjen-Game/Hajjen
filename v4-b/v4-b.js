(() => {
  ...
  const state={
    ...
    steadySteps:0,spawnTimers:[],spawnSerial:0,craftSelection:[]
  };
  ...
  add(1,5,{type:'shrine',mark:'✦',title:'VERDANT SHRINE',desc:'A quiet landmark. In V4-B it currently marks the route.'});
  add(8,3,{type:'shrine',mark:'✦',title:'MARSH SHRINE',desc:'A quiet landmark. In V4-B it currently marks the route.'});
  ...
  function renderBoard(){
    for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){
      ...
      if(isAdjacent(r,c)&&!state.gameOver&&!state.combat)tile.classList.add('reachable');
  ...
  function showTile(r,c){
    ...
    else if(e.type==='mob'||e.type==='elite'){$('tileSub').textContent=e.type==='elite'?'Elite encounter':'Mob encounter';$('tileDesc').textContent=`Base ${e.baseHp} HP / ${e.baseAttack} damage. Current Danger scaling: ${Math.round(enemyScale()*100)}%.`;}
  ...
  function attemptMove(r,c){
    if(state.gameOver||state.combat||!isAdjacent(r,c))return;
  ...
  function startCombat(entity,fromAggro=false){
    ...
    $('combatMessage').textContent=`Danger scaling ${Math.round(scale*100)}%. Choose a spell.`;
  ...
  function finishCombatWin(){
    ...
    if(dangerTier()==='hostile'||dangerTier()==='critical')maybeBackToBack();
  }
  ...
  function maybeBackToBack(){
    if(state.gameOver||state.combat)return;const chance=dangerTier()==='critical'?.60:.38;if(Math.random()>chance)return;setTimeout(()=>{log('Hostile Danger triggered a reinforcement attempt.','danger');requestSpawn(Math.random()<.18?'elite':'mob',true);},500);
  }

  function checkDangerAggro(){
    if(state.danger<10||state.combat||state.gameOver)return;
  ...
  window.addEventListener('keydown',e=>{
    if(state.combat||craftModal.classList.contains('show')||state.gameOver)return;const tag=document.activeElement?.tagName;if(tag==='INPUT'||tag==='TEXTAREA'||tag==='SELECT')return;
  ...
  makeBoard();renderAll();log('V4-B started: movement is free. Danger now reacts to time, harvesting and combat.','system');log('Defeat 4 mobs + 2 elites to unlock Rootmaw.','system');toast('MOVE FREELY — WASD OR CLICK','reward');
})();
