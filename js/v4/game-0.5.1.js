(() => {
  ...
  ingredients.push(
    {...ingredientPool.find(x => x.name === 'Bloomcap'), source:'Zone 1 starter'},
    {...ingredientPool.find(x => x.name === 'Cinder Seed'), source:'Zone 1 starter'}
  );
  ...
  const special = new Map([
    ['0,1',{type:'shrine',mark:'✦',title:'ANCIENT SHRINE',sub:'Verdant Brook',desc:'Land here to activate the shrine.'}],
    ['6,7',{type:'shrine',mark:'✦',title:'ANCIENT SHRINE',sub:'Verdant Brook',desc:'Land here to activate the shrine.'}],
    ['2,5',{type:'guardian',mark:'⚔',title:'VERDANT GUARDIAN',sub:'Elite Encounter',desc:'A dangerous elite. Triggers combat when entered or crossed.',enemy:{name:'Verdant Guardian',hp:130,attack:20}}],
    ['7,2',{type:'guardian',mark:'⚔',title:'MARSH GUARDIAN',sub:'Elite Encounter',desc:'A dangerous elite. Triggers combat when entered or crossed.',enemy:{name:'Marsh Guardian',hp:130,attack:20}}],
    ['1,5',{type:'mob',mark:'●',title:'BOGLING',sub:'Mob Encounter',desc:'Triggers combat when entered or crossed.',enemy:{name:'Bogling',hp:34,attack:8}}],
    ['8,8',{type:'mob',mark:'●',title:'ROOT STALKER',sub:'Mob Encounter',desc:'Triggers combat when entered or crossed.',enemy:{name:'Root Stalker',hp:40,attack:9}}],
    ['8,5',{type:'mob',mark:'●',title:'MIRELING',sub:'Mob Encounter',desc:'Triggers combat when entered or crossed.',enemy:{name:'Mireling',hp:36,attack:8}}],
    ...
    ['1,8',{type:'treasure',mark:'◆',title:'TREASURE',sub:'Reward Tile',desc:'Prototype: treasure placeholder.'}],
    ['9,9',{type:'boss',mark:'♛',title:'ROOTMAW',sub:'Verdant Boss',desc:'A heavy boss with strong direct attacks.',extra:'Weakness: Unknown',enemy:{name:'Rootmaw',hp:170,attack:19}}],
    ['0,4',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}],
    ['2,1',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}],
    ['3,8',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}],
    ['5,5',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}],
    ['7,6',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}],
    ['9,3',{type:'ingredient',mark:'✿',title:'INGREDIENT SITE',sub:'Primal Resources',desc:'Land here to reveal 3 ingredients and choose 1.'}]
  ]);
  ...
  function showTile(r,c){
    ...
    else if(s.type==='boss') tileExtra.textContent=`${bossUnlocked?'Boss Shrine: READY':'Boss Shrine: LOCKED'} · ${s.enemy.hp} HP · ${s.enemy.attack} damage per attack.`;
    else if(s.type==='ingredient') tileExtra.textContent='Standard: reveal 3 → choose 1. Manipulation Cards can change this.';
    else if(s.type==='guardian') tileExtra.textContent=`ELITE · ${s.enemy.hp} HP · ${s.enemy.attack} damage per attack.${danger>=10?' Danger 10+: can aggro from adjacent tiles.':' Direct contact/crossing triggers combat.'}`;
    else if(s.type==='mob') tileExtra.textContent=`${s.enemy.hp} HP · ${s.enemy.attack} damage per attack.${danger>=10?' Danger 10+: can aggro from adjacent tiles.':' Direct contact/crossing triggers combat.'}`;
  ...
  startZoneBtn.addEventListener('click',()=>{
    ...
    logline.textContent='Zonen har börjat. Du startar med Bloomcap + Cinder Seed. Kasta tärningarna.';
  });
  ...
  function spawnMob(){
    ...
    special.set(`${r},${c}`,{type:'mob',mark:'●',title:enemy.name.toUpperCase(),sub:'Spawned Mob Encounter',desc:'The disturbed zone has produced a new mob. Triggers combat when entered or crossed.',enemy:{...enemy,spawnId:spawnSerial}});
  ...
  async function resolveIngredientTile(source='landed'){
    ...
    chosen.forEach(ing=>ingredients.push({...ing,source:'Collected in Zone 1'}));lootRevealBonus=0;lootPickBonus=0;clearIngredientManipulations();
  ...
  function eligibleSpellsForEnhancement(card){return spellLibrary.filter(spell=>(!card.force||spell.force===card.force)&&spell.damage>0);}
  ...
  function openSpellbook(targetSlot=null,fromCombatPrep=false){
    ...
    const ingredientList=ingredients.length?`<ul class="ingredient-list">${ingredients.map(i=>`<li><strong>${i.name}</strong> (${i.force})${i.source?` — ${i.source}`:''}</li>`).join('')}</ul>`:'<p>No ingredients collected yet.</p>';
    ...
    showInfo('SPELLBOOK',`<p>${targetText}</p><div class="inventory-item"><h3>${spell.name}</h3><p>${spell.desc}</p><p><strong>Current direct damage:</strong> ${spellDamage(spell)}</p>${enhancementText}<div class="inventory-status">${equippedIndex>=0?`Currently equipped in Spell Slot ${equippedIndex+1}.`:'Not equipped.'}</div><div class="inventory-actions">${targetSlot!==null?`<button data-spell-equip="${targetSlot}">EQUIP TO SLOT ${targetSlot+1}</button>`:''}${equippedIndex>=0?`<button data-spell-unequip="${equippedIndex}">UNEQUIP</button>`:''}</div></div><p><strong>Ingredients:</strong></p>${ingredientList}<p class="system-note">Zone 1 starts with two ingredients so the first additional spell will require less early gathering once spell creation is implemented.</p><p class="system-note">Ingredient-strengthening effects are reserved for persistent loot carried in the Backpack between zones, not Enhancement Cards.</p>`);
  }
  ...
  function beginCombatPrep(encounter){
    ...
    const eliteWarning=encounter.tile.type==='guardian'?` ELITE: ${encounter.tile.enemy.hp} HP / ${encounter.tile.enemy.attack} damage per attack. This fight is intended to require real preparation.`:'';
    $('combatPrepText').textContent=(encounter.reason==='DIRECT CONTACT'?'You entered or crossed the enemy tile.':'Danger has activated an adjacent enemy.')+` Your current Spell/Action loadout is carried into combat; you may change it before starting.${eliteWarning}`;
  ...
  $('helpBtn').addEventListener('click',()=>showInfo('HAJJEN V4 — HELP',`
    ...
    <p><strong>Zone 1 start:</strong> Sharkan begins with two ingredients — Bloomcap and Cinder Seed — so the first new spell will need less gathering once spell creation is implemented.</p>
    <p><strong>Elite Guardians:</strong> Guardians are intentionally much stronger than normal mobs (130 HP / 20 damage in this balance pass) and are meant to punish fighting them before Sharkan is prepared.</p>
